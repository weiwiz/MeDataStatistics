...
var dailyDataReduce = function (key, values) {
    ...
    if ("050608070001" === record.type) {
        ...
        record.data.dis_temp = sumTemp / dataArrayLength;
    }
        ...
        record.data.power = sumPower / dataArrayLength;
        if (dataArrayLength > 1) {
            record.data.energyUsed = values[dataArrayLength - 1].data.energyUsed - values[0].data.energyUsed;
            record.data.energySaved = values[dataArrayLength - 1].data.energySaved - values[0].data.energySaved;
        }
    }
        ...
        record.data.power = (sumEffectiveVolt / dataArrayLength) * (sumEffectiveCurrent / dataArrayLength) / 100;
        delete record.data.effectiveVolt;
        delete record.data.effectiveCurrent;
        delete record.data.direct;
        if (dataArrayLength > 1) {
            record.data.toGrid = values[dataArrayLength - 1].data.toGrid - values[0].data.toGrid;
            record.data.toUser = values[dataArrayLength - 1].data.toUser - values[0].data.toUser;
        }
    }
        ...
        record.data.pacToGrid = sumPacToGrid / dataArrayLength;
        record.data.pacToUser = sumPacToUser / dataArrayLength;
        if (dataArrayLength > 1) {
            record.data.eDisChargeTotal = values[dataArrayLength - 1].data.eDisChargeTotal - values[0].data.eDisChargeTotal;
            record.data.eChargeTotal = values[dataArrayLength - 1].data.eChargeTotal - values[0].data.eChargeTotal;
            record.data.eToGridTotal = values[dataArrayLength - 1].data.eToGridTotal - values[0].data.eToGridTotal;
        }
    }
        ...
        record.data.currentPower = sumCurrentPower / dataArrayLength;
        if (dataArrayLength > 1) {
            record.data.totalEnergy = values[dataArrayLength - 1].data.totalEnergy - values[0].data.totalEnergy;
        }
    }
    record.length = dataArrayLength;
    record.values_ = JSON.stringify(values);
    return record;
};
...
dataStatistics.prototype.init = function () {
    ...
    db.once('open', function () {
        ...
        self.statisticsAll();
        /*self.scheduleJob = schedule.scheduleJob("5 * * * *", function () {
            self.statisticsAll();
        });*/
    });
};
