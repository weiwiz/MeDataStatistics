...
var REPORT_DATA_TYPE = ["dailyReport", "monthlyReport", "annualReport", "allReport"];//["dailyReport", "monthlyReport", "annualReport", "allReport"];
...
var dailyDataReduce = function (key, values) {
  ...
  if ("050608070001" === record.type) {
    ...
    record.data.dis_temp = sumTemp / len;
  }
    ...
    if (len > 1) {
      record.data.energyUsed = values[len - 1].data.energyUsed - values[0].data.energyUsed;
      record.data.energySaved = values[len - 1].data.energySaved - values[0].data.energySaved;
    }
  }
    ...
    record.data.power = (sumEffectiveVolt / len) * (sumEffectiveCurrent / len) / 100;
    delete record.data.effectiveVolt;
    delete record.data.effectiveCurrent;
    delete record.data.direct;
    if (len > 1) {
      record.data.toGrid = values[len - 1].data.toGrid - values[0].data.toGrid;
      record.data.toUser = values[len - 1].data.toUser - values[0].data.toUser;
    }
  }
    ...
    if (len > 1) {
      record.data.eDisChargeTotal = values[len - 1].data.eDisChargeTotal - values[0].data.eDisChargeTotal;
      record.data.eChargeTotal = values[len - 1].data.eChargeTotal - values[0].data.eChargeTotal;
      record.data.eToGridTotal = values[len - 1].data.eToGridTotal - values[0].data.eToGridTotal;
    }
  }
    ...
    if (len > 1) {
      record.data.totalEnergy = values[len - 1].data.totalEnergy - values[0].data.totalEnergy;
    }
  }
  /*  record.debug_={
        len:len,
...
var dataReduce = function (key, values) {
  ...
  else if ("010100000000" === record.type || "060A08000000" === record.type) {
  ...
  else if ("010001000004" === record.type || "01000C000004" === record.type || "01000D000004" === record.type) {
  ...
  else if ("04110E0E0001" === record.type) {
  ...
  else if ("04110F0F0001" === record.type) {
    var sumWindSpeed = 0;
    var sumRainfall = 0;
    var sumIrradiance = 0;
